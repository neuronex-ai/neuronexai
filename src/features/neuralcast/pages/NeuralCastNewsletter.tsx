      ...
      <NeuralCastShell>
        <section className="px-5 py-28 md:px-8 md:py-40">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <Link to="/neuralcast/newsletter" className="mt-9 inline-flex h-14 items-center rounded-full bg-[#17130f] px-7 text-xs font-black uppercase tracking-[0.12em] text-[#f4e8d1]"><ArrowLeft className="mr-2 h-4 w-4" /> Voltar aos artigos</Link>
          </div>
        </section>
      </NeuralCastShell>
    );
      ...
      <article>
        <header className="px-5 pb-20 pt-16 md:px-8 md:pb-28 md:pt-24">
          <div className="mx-auto max-w-[1120px]">
            <Link to="/neuralcast/newsletter" className="inline-flex items-center text-xs font-black uppercase tracking-[0.12em] text-[#17130f]/[0.60] transition-colors hover:text-[#17130f]"><ArrowLeft className="mr-2 h-4 w-4" /> Arquivo de artigos</Link>
        ...
        {article.quote ? <section className="bg-[#17130f] px-5 py-20 text-[#f4e8d1] md:px-8 md:py-28"><blockquote className="mx-auto max-w-[1120px] text-balance text-4xl font-black leading-[0.98] tracking-[-0.055em] md:text-6xl">“{article.quote}”</blockquote></section> : null}
        ...
        <section id="newsletter" className="bg-[#17130f] px-5 py-20 text-[#f4e8d1] md:px-8 md:py-28"><div className="mx-auto grid max-w-[1120px] gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:items-end"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#d5ad69]">Newsletter NeuralCast</p><h2 className="mt-5 max-w-[11ch] text-balance text-4xl font-black leading-[0.94] tracking-[-0.055em] md:text-6xl">Receba o próximo texto.</h2></div><div><p className="text-base font-medium leading-relaxed text-[#f4e8d1]/[0.58]">Artigos autorais sobre liderança, comportamento, cultura e relações de trabalho.</p><NeuralCastSubscribeForm inverted /></div></div></section>

        {related.length ? <section className="px-5 py-20 md:px-8 md:py-28"><div className="mx-auto max-w-[1120px]"><div className="flex items-end justify-between gap-5"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#17130f]/[0.45]">Continue lendo</p><h2 className="mt-5 text-4xl font-black tracking-[-0.055em] md:text-6xl">Outras reflexões.</h2></div><Link to="/neuralcast/newsletter" className="hidden items-center text-xs font-black uppercase tracking-[0.12em] md:inline-flex">Ver arquivo <ArrowRight className="ml-2 h-4 w-4" /></Link></div><div className="mt-10 grid gap-5 lg:grid-cols-2">{related.map((item) => <NeuralCastArticleCard key={item.slug} article={item} />)}</div></div></section> : null}
      </article>
    </NeuralCastShell>
  );
}
...
function NeuralCastArchive() {
  ...
  return (
    <NeuralCastShell>
      ...
      <section className="border-y border-[#17130f]/[0.12] bg-[#f4e8d1]/[0.28] px-5 py-8 md:px-8"><div className="mx-auto flex max-w-[1320px] flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div className="flex flex-wrap gap-2">{NEURALCAST_CATEGORIES.map((item) => <button key={item} type="button" onClick={() => setCategory(item)} className={cn("h-10 rounded-full border px-4 text-xs font-black transition-colors", category === item ? "border-[#17130f] bg-[#17130f] text-[#f4e8d1]" : "border-[#17130f]/[0.15] bg-transparent text-[#17130f]/[0.62] hover:border-[#17130f]/[0.35] hover:text-[#17130f]")}>{item}</button>)}</div><label className="relative block w-full lg:max-w-sm"><span className="sr-only">Buscar artigos</span><Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#17130f]/[0.42]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por tema ou palavra" className="h-12 w-full rounded-full border border-[#17130f]/[0.15] bg-[#ead8b7] pl-11 pr-5 text-sm font-semibold outline-none focus:ring-2 focus:ring-[#17130f]/[0.15]" /></label></div></section>

      <section className="px-5 py-20 md:px-8 md:py-28"><div className="mx-auto max-w-[1320px]"><div className="mb-8 flex items-center justify-between gap-4"><p className="text-sm font-black">{filteredArticles.length} {filteredArticles.length === 1 ? "artigo encontrado" : "artigos encontrados"}</p><p className="text-xs font-semibold text-[#17130f]/[0.45]">Arquivo em expansão</p></div>{filteredArticles.length ? <div className="grid gap-5 lg:grid-cols-2">{filteredArticles.map((article) => <NeuralCastArticleCard key={article.slug} article={article} />)}</div> : <div className="rounded-[32px] border border-[#17130f]/[0.12] bg-[#f4e8d1]/[0.35] px-6 py-20 text-center"><h2 className="text-3xl font-black tracking-[-0.045em]">Nenhum texto encontrado.</h2><p className="mt-3 text-sm font-semibold text-[#17130f]/[0.55]">Tente outra palavra ou volte para a categoria “Todos”.</p></div>}</div></section>

      <section id="newsletter" className="bg-[#17130f] px-5 py-24 text-[#f4e8d1] md:px-8 md:py-32"><div className="mx-auto grid max-w-[1320px] gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-end"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#d5ad69]">Assine a NeuralCast</p><h2 className="mt-5 max-w-[12ch] text-balance text-5xl font-black leading-[0.9] tracking-[-0.065em] md:text-7xl">O próximo artigo pode chegar até você.</h2></div><div><p className="max-w-2xl text-lg font-medium leading-relaxed text-[#f4e8d1]/[0.58]">Uma seleção autoral, sem ruído e sem excesso de envios.</p><NeuralCastSubscribeForm inverted /></div></div></section>
    </NeuralCastShell>
  );
}
