        ...
        <div className="mx-auto grid min-h-[calc(100svh-8rem)] max-w-[1320px] items-center gap-12 lg:grid-cols-[1.12fr_0.88fr] lg:gap-16">
          <div className="relative z-10 py-8">
            ...
            <div className="mt-10 flex flex-col gap-3 sm:flex-row">
              <Link to="/neuralcast/newsletter" className="inline-flex h-14 items-center justify-center rounded-full bg-[#17130f] px-7 text-xs font-black uppercase tracking-[0.12em] text-[#f4e8d1] transition-transform hover:-translate-y-0.5">
                Explorar artigos <ArrowRight className="ml-2 h-4 w-4" />
              </Link>
              <a href="#manifesto" className="inline-flex h-14 items-center justify-center rounded-full border border-[#17130f]/[0.18] bg-[#f4e8d1]/[0.35] px-7 text-xs font-black uppercase tracking-[0.12em] transition-colors hover:bg-[#f4e8d1]/[0.70]">
        ...
        <div className="mx-auto max-w-[1320px]">
          ...
          <div className="mt-12 grid gap-8 rounded-[36px] border border-[#f4e8d1]/[0.14] bg-[#f4e8d1]/[0.035] p-7 md:grid-cols-[1.1fr_0.9fr] md:p-12">
            ...
            <div className="flex flex-col justify-between border-t border-[#f4e8d1]/[0.14] pt-7 md:border-l md:border-t-0 md:pl-9 md:pt-0">
              ...
              <Link to={`/neuralcast/newsletter/${featuredArticle.slug}`} className="mt-10 inline-flex h-14 items-center justify-center rounded-full bg-[#f4e8d1] px-7 text-xs font-black uppercase tracking-[0.12em] text-[#17130f] transition-transform hover:-translate-y-0.5">Ler artigo completo <ArrowRight className="ml-2 h-4 w-4" /></Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-24 md:px-8 md:py-36">
        <div className="mx-auto max-w-[1320px]">
          <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            ...
            <Link to="/neuralcast/newsletter" className="inline-flex items-center text-xs font-black uppercase tracking-[0.12em]">Ver todos os artigos <ArrowRight className="ml-2 h-4 w-4" /></Link>
          </div>
    ...
    <>
      <section className="relative overflow-hidden px-4 pb-14 pt-10">
        ...
        <div className="relative">
          ...
          <h1 className="mt-7 text-balance text-[clamp(3.75rem,19vw,5.75rem)] font-black leading-[0.82] tracking-[-0.078em]">Ideias que transformam a forma de liderar.</h1>
          ...
          <div className="mt-8 grid gap-3">
            <Link to="/neuralcast/newsletter" className="inline-flex h-14 items-center justify-center rounded-full bg-[#17130f] px-6 text-xs font-black uppercase tracking-[0.11em] text-[#f4e8d1]">Explorar artigos <ArrowRight className="ml-2 h-4 w-4" /></Link>
            <a href="#mobile-manifesto" className="inline-flex h-14 items-center justify-center rounded-full border border-[#17130f]/[0.18] bg-[#f4e8d1]/[0.38] px-6 text-xs font-black uppercase tracking-[0.11em]">Entender a proposta <ArrowDownRight className="ml-2 h-4 w-4" /></a>
          </div>
        </div>

        <div className="relative mt-10 overflow-hidden rounded-[34px] bg-[#17130f] p-6 text-[#f4e8d1] shadow-[0_30px_80px_rgba(23,19,15,0.24)]">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_15%,rgba(169,120,56,0.5),transparent_34%)]" />
          <div className="relative"><NeuralCastLogo surface="dark" compact /><Quote className="mt-16 h-8 w-8 text-[#d5ad69]" /><p className="mt-6 text-balance text-4xl font-black leading-[0.94] tracking-[-0.055em]">Pessoas não são peças. São sistemas vivos em desenvolvimento.</p><div className="mt-8 border-t border-[#f4e8d1]/[0.16] pt-5"><p className="font-black">Pedro Luiz Pereira</p><p className="mt-1 text-xs text-[#f4e8d1]/[0.55]">Liderança · comportamento · cultura</p></div></div>
        </div>
      </section>

      <section className="bg-[#17130f] px-4 py-12 text-[#f4e8d1]">
      ...
      <section id="mobile-manifesto" className="px-4 py-20"><CircleDot className="h-7 w-7 text-[#a97838]" /><p className="mt-6 text-[9px] font-black uppercase tracking-[0.2em] text-[#17130f]/[0.45]">A proposta</p><h2 className="mt-5 text-balance text-5xl font-black leading-[0.9] tracking-[-0.065em]">Traduzir complexidade sem transformar pessoas em fórmulas.</h2><div className="mt-8 space-y-5 border-t border-[#17130f]/[0.15] pt-7 text-base font-semibold leading-relaxed text-[#17130f]/[0.66]"><p>A NeuralCast organiza ideias, provoca conversas e aproxima neurociência, comportamento e vida organizacional.</p><p>Sem receitas prontas. Cada texto parte de uma pergunta real e oferece repertório para decisões mais humanas e conscientes.</p></div></section>

      <section className="bg-[#17130f] px-4 py-20 text-[#f4e8d1]"><p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#d5ad69]">Artigo em destaque</p><h2 className="mt-5 text-balance text-5xl font-black leading-[0.9] tracking-[-0.065em]">{featuredArticle.title}</h2><blockquote className="mt-9 rounded-[28px] border border-[#f4e8d1]/[0.14] bg-[#f4e8d1]/[0.04] p-6 text-3xl font-black leading-[1.02] tracking-[-0.045em]">“{featuredArticle.quote}”</blockquote><Link to={`/neuralcast/newsletter/${featuredArticle.slug}`} className="mt-7 inline-flex h-14 w-full items-center justify-center rounded-full bg-[#f4e8d1] px-6 text-xs font-black uppercase tracking-[0.11em] text-[#17130f]">Ler artigo completo <ArrowRight className="ml-2 h-4 w-4" /></Link></section>
