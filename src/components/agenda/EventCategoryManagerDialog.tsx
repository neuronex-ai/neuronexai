...
export function EventCategoryManagerDialog({
  ...
  const handleUpdate = async (id: string) => {
    try {
      const category = await updateCategory({ id, rawName: editingName });
      setEditingId(null);
      setEditingName("");
      onSelect(category.slug);
      ...
      >
        <DialogHeader className="border-b border-border/55 px-5 pb-4 pt-5 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle>Categoria do compromisso</DialogTitle>
              <DialogDescription className="mt-1">
                Organize os eventos com nomes que façam sentido para sua rotina.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="max-h-[min(34rem,calc(100dvh-11rem))] space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
          ...
          <section aria-labelledby="event-category-list-heading">
            ...
            {isLoading ? (
              <div
                className="agenda-liquid-card flex min-h-24 items-center justify-center gap-2 rounded-[20px] border text-sm text-muted-foreground"
                role="status"
              >
                <Loader2
                  className="h-4 w-4 animate-spin motion-reduce:animate-none"
            ...
            {isError ? (
              <p
                className="rounded-[18px] border border-destructive/25 bg-destructive/10 px-4 py-3 text-sm text-destructive"
                role="alert"
              >
                Não foi possível carregar as categorias agora. Feche esta janela
                e tente novamente.
              </p>
            ) : null}
              ...
              <ul className="space-y-2">
                {categories.map((category) => {
                  ...
                  return (
                    <li
                      key={category.id}
                      className="agenda-liquid-card rounded-[18px] border p-2.5"
                    >
                      {isConfirmingArchive ? (
                        <div className="flex min-h-11 items-center justify-between gap-3 px-1">
                          <p className="min-w-0 text-xs leading-relaxed text-muted-foreground">
                            Remover <strong className="text-foreground">{category.name}</strong>?
                            Eventos anteriores mantêm esse nome.
                          </p>
                        ...
                        <div className="flex min-h-11 items-center gap-2">
                          ...
                          <Button
                            ...
                            size="icon"
                            onClick={() =>
                              setConfirmingArchiveId(category.id)
                            }
