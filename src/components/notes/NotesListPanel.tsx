  ...
  if (isCollapsed) {
    return (
      <div className="relative flex h-full w-full flex-col items-center bg-transparent py-4">
        <button onClick={onToggleCollapsed} className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/[0.06] bg-white/[0.04] text-zinc-400 transition-all hover:bg-white/[0.09] hover:text-white active:scale-95 [.light_&]:border-zinc-200/70 [.light_&]:bg-white [.light_&]:text-zinc-500 [.light_&]:hover:bg-zinc-100 [.light_&]:hover:text-zinc-950" title="Expandir lista de notas">
        ...
        <button onClick={onCreate} disabled={isCreatingNote} className="mt-auto flex h-9 w-9 items-center justify-center rounded-xl bg-white text-zinc-950 transition-all hover:scale-105 active:scale-95 disabled:opacity-50 [.light_&]:bg-zinc-950 [.light_&]:text-white" title="Nova nota">
        ...
        <div className="relative z-10 shrink-0 space-y-5 border-b border-white/[0.045] px-5 pb-5 pt-6 [.light_&]:border-zinc-200/60">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={onCreate} disabled={isCreatingNote} className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-zinc-950 shadow-[0_15px_30px_-18px_rgba(255,255,255,0.45)] transition-all hover:-translate-y-0.5 active:translate-y-0 active:scale-95 disabled:opacity-50 [.light_&]:bg-zinc-950 [.light_&]:text-white [.light_&]:shadow-[0_15px_30px_-18px_rgba(0,0,0,0.35)]" title="Nova nota">
                <Plus className={cn("h-4 w-4", isCreatingNote && "animate-pulse")} strokeWidth={2.5} />
              </button>
              <button onClick={onToggleCollapsed} className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/[0.06] bg-white/[0.035] text-zinc-400 transition-all hover:bg-white/[0.08] hover:text-white active:scale-95 [.light_&]:border-zinc-200/70 [.light_&]:bg-white [.light_&]:text-zinc-500 [.light_&]:hover:bg-zinc-100 [.light_&]:hover:text-zinc-950" title="Recolher lista">
        ...
        <div className="notes-scroll-surface relative z-10 min-h-0 flex-1 space-y-2.5 overflow-y-auto overscroll-contain px-3 pb-4 pt-3 custom-scrollbar [scrollbar-gutter:stable]" data-synapse-target="notes-list">
          {isLoading ? (
            <div className="space-y-3">
              {[1, 2, 3, 4].map((item) => <div key={item} className="h-24 animate-pulse rounded-2xl border border-white/[0.04] bg-white/[0.025] [.light_&]:border-zinc-200/60 [.light_&]:bg-zinc-100/70" />)}
            </div>
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <button onClick={(event) => { event.stopPropagation(); setNoteToDelete({ id: item.id, title: item.title || "Nota sem título" }); }} className={cn("flex h-7 w-7 shrink-0 items-center justify-center rounded-lg opacity-0 transition-all group-hover:opacity-100", isActive ? "text-zinc-500 hover:bg-black/5 hover:text-red-500 [.light_&]:text-zinc-400 [.light_&]:hover:bg-white/10 [.light_&]:hover:text-red-300" : "text-zinc-600 hover:bg-white/[0.06] hover:text-red-400 [.light_&]:text-zinc-400 [.light_&]:hover:bg-red-50 [.light_&]:hover:text-red-500")} title="Excluir nota">
      ...
      <AlertDialog open={!!noteToDelete} onOpenChange={(open) => !open && setNoteToDelete(null)}>
        <AlertDialogContent className="rounded-[28px] border border-white/[0.08] bg-[#101012]/95 text-white shadow-2xl backdrop-blur-2xl [.light_&]:border-zinc-200 [.light_&]:bg-white [.light_&]:text-zinc-950">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-zinc-400 [.light_&]:text-zinc-600">Esta ação não pode ser desfeita. A nota “{noteToDelete?.title}” será removida.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
