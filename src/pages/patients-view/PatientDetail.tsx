...
const PATIENT_TABS = ["summary", "sessions", "anamnesis", "mood", "goals", "packages", "finance", "documents"] as const;

type PatientTabValue = (typeof PATIENT_TABS)[number];
...
function DesktopPatientDetail() {
    ...
    const [activeTab, setActiveTab] = useState<PatientTabValue>("summary");
    const [mountedTabs, setMountedTabs] = useState<Set<PatientTabValue>>(() => new Set(["summary"]));
    ...
    const previousTabRef = useRef<PatientTabValue>("summary");
    ...
    useEffect(() => {
        ...
        if (PATIENT_TABS.includes(compatibilityTab as PatientTabValue)) {
            setActiveTab(compatibilityTab as PatientTabValue);
        } else {
    ...
    useEffect(() => {
        ...
        if (scrollNode && previousTab !== activeTab) {
            tabScrollPositionsRef.current[previousTab] = scrollNode.scrollTop;
            window.requestAnimationFrame(() => {
                const nextNode = contentScrollRef.current;
                if (!nextNode) return;
                nextNode.scrollTop = tabScrollPositionsRef.current[activeTab] || 0;
            });
        }
    ...
    const handleTabChange = (tab: string, requestedSessionView?: "history" | "pending") => {
        if (!PATIENT_TABS.includes(tab as PatientTabValue)) return;
        ...
        setActiveTab(tab as PatientTabValue);
    ...
    const handleTabWheel = (event: React.WheelEvent<HTMLDivElement>) => {
        ...
        tabScrollRef.current.scrollLeft += event.deltaY;
    };
    ...
    if (!patient) {
        return (
            <div className="flex min-h-screen w-full flex-col items-center justify-center text-muted-foreground">
                <div className="mb-6 flex h-24 w-24 items-center justify-center rounded-3xl border border-border/10 bg-secondary/10">
                    <Shield className="h-10 w-10 opacity-20" />
                </div>
                <h2 className="mb-2 text-2xl font-bold text-foreground">Paciente não encontrado</h2>
                <Button variant="outline" onClick={() => navigate('/pacientes')} className="h-12 rounded-full border-border/10 px-8 hover:bg-secondary/10">
                    Voltar para Lista
                </Button>
            </div>
        );
    ...
    const patientTabs = [
        { val: "summary" as const, label: "Resumo", icon: Gauge },
        { val: "sessions" as const, label: "Sessões", icon: Layers3 },
        { val: "anamnesis" as const, label: "Anamneses", icon: ClipboardList },
        { val: "mood" as const, label: "Humor", icon: Smile },
        { val: "goals" as const, label: "Metas", icon: Target },
        { val: "packages" as const, label: "Planos", icon: Package },
        { val: "finance" as const, label: "Financeiro", icon: Wallet },
        { val: "documents" as const, label: "Arquivos", icon: FileOutput },
    ];
                    ...
                    <div className="flex w-full items-center gap-4">
                        ...
                        <TooltipProvider delayDuration={220}>
                            <div
                                ...
                                className="patient-record-tabbar desktop-retina-inset custom-premium-scrollbar flex min-w-0 flex-1 select-none items-center overflow-x-auto rounded-[20px] border border-border/55 bg-muted/30 p-1"
                            >
                                                ...
                                                <TooltipTrigger asChild>
                                                    <button
                                                        ...
                                                        onKeyDown={(event) => {
                                                            ...
                                                            const buttons = Array.from(tabScrollRef.current?.querySelectorAll<HTMLButtonElement>("[data-patient-record-tab]") || []);
                                                        ...
                                                        className={cn(
                                                            "desktop-retina-interactive relative flex h-11 items-center justify-center gap-2 whitespace-nowrap rounded-[15px] text-[9px] font-black uppercase tracking-[0.16em] transition-[width,padding,background-color,color] duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50",
                                                            isActive ? "bg-foreground px-4 text-background" : "w-11 px-0 text-muted-foreground hover:bg-background hover:text-foreground",
                                                        )}
                                                    >
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                            ...
                            <Select value={patient.status || ""} onValueChange={handleStatusChange}>
                                ...
                                <SelectContent align="end" className="w-[200px] rounded-3xl border-border bg-popover/95 p-2 shadow-2xl backdrop-blur-xl">
                                    <SelectItem value="active" className="rounded-2xl py-3 text-[10px] font-black uppercase tracking-widest">Paciente Ativo</SelectItem>
                                    <SelectItem value="inactive" className="rounded-2xl py-3 text-[10px] font-black uppercase tracking-widest">Inativo</SelectItem>
                                    <SelectItem value="archived" className="rounded-2xl py-3 text-[10px] font-black uppercase tracking-widest text-orange-500">Arquivado</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                    </div>
                ...
                <div className="patient-record-shell desktop-retina-frame relative z-10 mt-3 min-h-0 flex-1 overflow-hidden rounded-[36px] border border-border/45 bg-background/45 p-3 shadow-[0_28px_82px_-64px_hsl(var(--foreground)/0.4)] dark:bg-black/20">
                    <div className="grid h-full min-h-0 grid-cols-1 gap-3 md:grid-cols-[230px_minmax(0,1fr)] lg:grid-cols-[270px_minmax(0,1fr)] xl:grid-cols-[300px_minmax(0,1fr)]">
                        <aside className="patient-record-scrollbar z-20 min-h-0 w-full overflow-y-auto overscroll-contain border-b border-border/45 pr-1 md:border-b-0">
                            <section className="patient-record-panel w-full overflow-hidden rounded-[24px] border">
                                <div className="relative overflow-hidden p-5">
                                    ...
                                    <div className="mt-6 grid grid-cols-2 gap-2 border-t border-border/55 pt-5">
                                        ...
                                        <div className="space-y-3">
                                            {patient.medications && patient.medications.length > 0 ? (
                                                ...
                                                <div className="clinical-inset-surface rounded-2xl border border-dashed p-6 text-center">
                                                    <Pill className="mx-auto mb-3 h-5 w-5 text-zinc-300 opacity-30 dark:text-zinc-800" />
                                                    <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 italic">Nenhuma medicação</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                        ...
                        <main className="patient-record-content desktop-retina-frame min-h-0 min-w-0 overflow-hidden rounded-[30px] border border-border/45 bg-card/62">
                            <div ref={contentScrollRef} className="patient-record-scrollbar relative h-full min-h-0 w-full overflow-y-auto overscroll-contain p-4 [scrollbar-gutter:stable] md:p-6 lg:p-7">
                                <Tabs value={activeTab} onValueChange={handleTabChange} className="flex min-h-full w-full flex-col">
                                    <div className="min-h-full">
                                        <TabsContent forceMount data-synapse-target="patient-summary" id="patient-record-panel-summary" aria-labelledby="patient-record-tab-summary" value="summary" className="mt-0 focus-visible:outline-none data-[state=inactive]:hidden">
                                            {mountedTabs.has("summary") ? <PatientRecordSummaryTab patient={patient} patientId={id!} onNavigate={handleTabChange} /> : null}
                                        </TabsContent>
                                        <TabsContent forceMount data-synapse-target="patient-sessions" id="patient-record-panel-sessions" aria-labelledby="patient-record-tab-sessions" value="sessions" className="mt-0 focus-visible:outline-none data-[state=inactive]:hidden">
                                        ...
                                        <TabsContent forceMount id="patient-record-panel-anamnesis" aria-labelledby="patient-record-tab-anamnesis" value="anamnesis" className="mt-0 h-full focus-visible:outline-none data-[state=inactive]:hidden">
                                            {mountedTabs.has("anamnesis") ? <Suspense fallback={<PatientTabFallback />}><AnamnesisTab /></Suspense> : null}
                                        </TabsContent>
                                        <TabsContent forceMount id="patient-record-panel-mood" aria-labelledby="patient-record-tab-mood" value="mood" className="mt-0 h-full focus-visible:outline-none data-[state=inactive]:hidden">
                                            {mountedTabs.has("mood") ? <Suspense fallback={<PatientTabFallback />}><PatientMoodTab patientId={id!} /></Suspense> : null}
                                        </TabsContent>
                                        <TabsContent forceMount id="patient-record-panel-goals" aria-labelledby="patient-record-tab-goals" value="goals" className="mt-0 h-full focus-visible:outline-none data-[state=inactive]:hidden">
                                            {mountedTabs.has("goals") ? <Suspense fallback={<PatientTabFallback />}><PatientGoalsTab patientId={id!} /></Suspense> : null}
                                        </TabsContent>
                                        <TabsContent forceMount id="patient-record-panel-packages" aria-labelledby="patient-record-tab-packages" value="packages" className="mt-0 h-full focus-visible:outline-none data-[state=inactive]:hidden">
                                            {mountedTabs.has("packages") ? <Suspense fallback={<PatientTabFallback />}><PatientPackagesTab patientId={id!} /></Suspense> : null}
                                        </TabsContent>
                                        <TabsContent forceMount data-synapse-target="patient-finance" id="patient-record-panel-finance" aria-labelledby="patient-record-tab-finance" value="finance" className="mt-0 h-full focus-visible:outline-none data-[state=inactive]:hidden">
                                            {mountedTabs.has("finance") ? <Suspense fallback={<PatientTabFallback />}><PatientFinanceTab patientId={id!} /></Suspense> : null}
                                        </TabsContent>
                                        <TabsContent forceMount data-synapse-target="patient-files" id="patient-record-panel-documents" aria-labelledby="patient-record-tab-documents" value="documents" className="mt-0 h-full focus-visible:outline-none data-[state=inactive]:hidden">
                                            {mountedTabs.has("documents") ? <Suspense fallback={<PatientTabFallback />}><PatientDocumentsTab patientId={id!} /></Suspense> : null}
                                        </TabsContent>
                                    </div>
                                </Tabs>
                            </div>
