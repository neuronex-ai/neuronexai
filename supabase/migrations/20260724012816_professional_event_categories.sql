...
create policy "Professional event categories delete own"
  on public.professional_event_categories
  for delete
  to authenticated
  using ((select auth.uid()) = professional_id);

revoke all on table public.professional_event_categories from anon, authenticated;
grant select, insert, delete on table public.professional_event_categories to authenticated;
grant update (name, is_archived)
  on table public.professional_event_categories
  to authenticated;
...
comment on table public.professional_event_categories is
  'Categorias próprias de compromissos da agenda profissional. Exclusões no produto são arquivamentos para preservar a leitura de eventos históricos.';
