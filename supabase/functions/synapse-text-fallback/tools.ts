...
export const AGENT_TOOLS = [
  ...
  fn("create_appointment", "Prepara um plano imutável de agendamento simples ou recorrente na Agenda Desktop e exige confirmação separada. Sempre consulte horários livres antes quando o profissional não indicar horário exato.", objectSchema({ ...patientReference, datetime: { type: "string", description: "Data/hora local de Brasília em ISO" }, duration_minutes: { type: "integer", minimum: 15, maximum: 240 }, appointment_type: { type: "string", enum: ["presencial", "online", "block"] }, frequency: { type: "string", enum: ["single", "weekly", "biweekly", "monthly"] }, occurrence_count: { type: "integer", minimum: 1, maximum: 20 }, notes: { type: "string" }, location: { type: "string" }, package_id: { type: "string", description: "Vínculo interno opcional já resolvido." }, financial_mode: { type: "string", enum: ["none", "manual", "neurofinance", "package"] }, value_per_session: { type: "number", minimum: 0 }, total: { type: "number", minimum: 0 }, charge_mode: { type: "string", enum: ["per_occurrence", "series"] }, send_confirmation: { type: "boolean" }, communication_provider: { type: "string", enum: ["configured", "gmail", "neuronex"] }, reminder_policy: { type: "string" }, fiscal_automation_enabled: { type: "boolean" }, fiscal_trigger: { type: "string" } }, ["patient_name", "datetime"])),
